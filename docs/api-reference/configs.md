...
description: Complete configuration schemas for training with Hydra and GRPOConfig.
---
...
The repository uses Hydra for configuration management with composition and overrides.

### Directory Structure

```
configs/
├── run/                    # Complete run configurations
│   ├── teacher_sft.yaml   # SFT warmup configuration
│   └── teacher_rcl.yaml   # RL training configuration
├── model/                  # Model configurations
│   └── qwen3_8b.yaml      # Model-specific settings
├── data/                   # Dataset configurations
│   └── arc_atlas_sft.yaml # Dataset loading settings
├── trainer/                # Trainer configurations
│   ├── reward/            # Reward function configs
│   │   └── adaptive_teaching.yaml
│   └── teacher_grpo.yaml  # GRPO trainer settings
└── train.yaml             # Base configuration
```

### Base Configuration (train.yaml)
...
# Global settings
seed: 42
output_dir: results/${model.name}_${now:%Y%m%d_%H%M%S}
report_to: wandb
logging_steps: 10
...
### Run Configuration Example (teacher_rcl.yaml)
...
defaults:
  ...
  - override /trainer/reward: adaptive_teaching
...
### Reward Configuration (adaptive_teaching.yaml)
...
teacher_adaptive_template: |
  ...
  The student's initial approach:
  {approach}
  
  Based on the student's approach, provide adaptive teaching...
